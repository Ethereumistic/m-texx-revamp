...
// Import all logo arrays and combine them
import { logos, logoz, logox } from "@/lib/logos"

// Combine all logos into a single array
const allLogos = [...logos, ...logoz, ...logox]

// Define regions for categorization
const regions = [
  { id: "all", name: "Всички", count: allLogos.length },
  { id: "north", name: "Северна България", count: allLogos.filter((logo) => logo.region === "north").length },
  { id: "south", name: "Южна България", count: allLogos.filter((logo) => logo.region === "south").length },
  { id: "central", name: "Централна България", count: allLogos.filter((logo) => logo.region === "central").length },
  { id: "east", name: "Източна България", count: allLogos.filter((logo) => logo.region === "east").length },
  { id: "west", name: "Западна България", count: allLogos.filter((logo) => logo.region === "west").length },
]
...
export function Partners() {
  ...
  const groupedLogos = useMemo(() => {
    const groups: Record<string, typeof allLogos> = {}
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
            Работим с {allLogos.length} общини в България за устойчиво рециклиране на текстил и намаляване на
            отпадъците.
          </p>
        </motion.div>
        ...
        <div className="mb-6 flex justify-between items-center">
          <p className="text-sm text-muted-foreground">
            Показани {visibleLogos.length} от {allLogos.length} общини
          </p>
          {searchQuery && (
            <p className="text-sm">
